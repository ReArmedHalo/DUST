Function Get-AdminMailForwarding {
    [CmdletBinding()]Param(
        [Parameter()]
    ...
    # Admin rules can be set by the user in Mail Forwarding options of OWA or by an admin in Office 365 admin center
    $adminForwarding = $mailboxesToSearch | Where-Object {$_.ForwardingSmtpAddress -gt ''} | Select-Object UserPrincipalName,ForwardingSmtpAddress,DeliverToMailboxAndForward

    ForEach ($rule in $adminForwarding) {
        $forwarding = [AdminMailForwardingRule]@{
