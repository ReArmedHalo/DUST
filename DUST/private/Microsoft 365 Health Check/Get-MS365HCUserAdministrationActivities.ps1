Function Get-MS365HCUserAdministrationActivities {
    ...
    try {
        $graphRequestUri = "https://graph.microsoft.com/v1.0/auditlogs/directoryaudits?`$filter=activityDateTime gt $StartDate and loggedByService eq 'Core Directory' and category eq 'UserManagement' and (activityDisplayName eq 'Add user' or activityDisplayName eq 'Delete user')"
